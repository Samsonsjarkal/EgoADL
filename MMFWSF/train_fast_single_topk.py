...
class Captioning(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        if ('audio' in self.hparams.modality):
            ...
            src_audioslow = self.hparams.audio_slow_CNN(feats_audio)
        ...
        # print(src_audioslow.size())
        # src_audioslow = src_audioslow.contiguous().view(src_audioslow.size(0),src_audioslow.size(1), -1)
        src_modalitywise = src_list[0]
        for i in range(1, len(src_list)):
            src_modalitywise = torch.cat((src_modalitywise,src_list[i]),dim = 1)

        src_modalitywise = src_modalitywise.contiguous().view(src_modalitywise.size(0),src_modalitywise.size(1), -1)
        ...
        src = src_modalitywise
        # src = torch.cat((src_audioslow, src_modalitywise), dim = 1)
        # src = torch.cat((src, src_framewise), dim = 1)
        # print(src.size())
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if current_epoch % valid_search_interval == 0 or (
                ...
                for predicted_word in predicted_words:
                    ...
                    self.wer_metric.append(ids, [predicted_word], target_words)
...
if __name__ == "__main__":
    ...
    cap_brain.hparams.wer_file = os.path.join(
        hparams["output_folder"], "wer_{}.txt".format('100_test_topk')
    )
    cap_brain.evaluate(
        test_set,
        max_key="ACC",
